...
// A guest frame is counted when the title submits an XE_SWAP packet. This is
// deliberately separate from host presentation rate, which can be much higher
// when VSync is disabled.
struct GuestSwapStats {
...
class GraphicsSystem : public system::IGraphicsSystem {
 public:
  ...
  void NotifyGuestSwap();
  GuestSwapStats guest_swap_stats() const;
 ...
 protected:
  ...
  std::atomic<uint64_t> guest_swap_fps_milli_{0};
  std::atomic<uint64_t> guest_swap_frame_time_us_{0};
  uint64_t guest_swap_last_tick_ = 0;
  uint64_t guest_swap_window_start_tick_ = 0;
  uint64_t guest_swap_window_count_ = 0;

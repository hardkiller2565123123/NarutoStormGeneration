...
void BuilderContext::emit_function_call(uint32_t address) {
  ...
  // Overlapping configured chunks can own the same instruction through more
  // than one FunctionNode. In that case the target can be registered even
  // though this node did not receive the corresponding CallEdge.
  ...
  // No pre-resolved or registered target found - this is an error
  ...
  switch (kind) {
    ...
    case TargetKind::Import:
      ...
      if (const auto* callTarget = findCallTarget(base)) {
        ...
        // See emit_function_call: shared-tail chunks may be registered without
        // an edge on every overlapping owner.

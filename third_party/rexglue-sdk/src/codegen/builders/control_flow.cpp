...
bool build_bctr(BuilderContext& ctx) {
  ...
  if (jt) {
    // The table scan records guest code addresses, but the register used to
    // derive the table index is often overwritten by the load that supplies
    // CTR. Dispatch on the final branch target, which is what bctr observes.
    ...
    ctx.println("\tdefault:");
    // Some games fill virtual tables after startup, so a valid target may not
    // appear in the static image. Route those through the runtime table.

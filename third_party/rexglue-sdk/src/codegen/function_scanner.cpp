...
FunctionBlocks FunctionScanner::discover_blocks(rex::guest_addr_t entry_point,
  ...
  while (!block_stack.empty() && result.blocks.size() < MAX_BLOCKS) {
    ...
    if (instr.opcode == Opcode::bcctr) {
      // Conditional forms such as bnectr branch through CTR only when the
      // condition is true. The false path continues at the next instruction.
      ...
      if (isBranch(*insn)) {
        ...
        if (isCall(*insn)) {
          ...
          break;
                   ...
                   isConditional(*insn)) {
          // Conditional CTR branches have a reachable false path. They are
          // not jump tables, and the fallthrough must be discovered as code.

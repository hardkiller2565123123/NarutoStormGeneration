...
std::unique_ptr<FileHandle> FileHandle::OpenExisting(const std::filesystem::path& path,
  ...
  DWORD file_flags = FILE_ATTRIBUTE_NORMAL | FILE_FLAG_BACKUP_SEMANTICS;
  // Large CPK archives are accessed through many offset-based reads. Tell the
  // Windows cache manager not to assume strictly sequential access, allowing
  // it to retain useful archive pages instead of aggressively discarding them.
  if ((open_access & GENERIC_READ) && !(open_access & GENERIC_WRITE)) {

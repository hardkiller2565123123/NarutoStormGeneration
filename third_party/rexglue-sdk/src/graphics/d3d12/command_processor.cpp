...
REXCVAR_DEFINE_INT32(d3d12_debug_swap_trace_interval, 0, "GPU/D3D12",
                     "Log every Nth guest swap for a renderer diagnostic run")
    .range(0, 1000000);

REXCVAR_DEFINE_STRING(d3d12_debug_swap_capture_path, "", "GPU/D3D12",
                      "Write one pre-presentation R8G8B8A8 guest swap source to this BMP path");

REXCVAR_DEFINE_INT32(d3d12_debug_swap_capture_after, 0, "GPU/D3D12",
                     "Capture the pre-presentation guest swap source after this many swaps")
    .range(0, 1000000);

REXCVAR_DEFINE_INT32(d3d12_debug_swap_capture_interval, 0, "GPU/D3D12",
                     "Capture additional pre-presentation guest swap sources every N swaps")
    .range(0, 1000000);

REXCVAR_DEFINE_INT32(d3d12_debug_swap_capture_limit, 1, "GPU/D3D12",
                     "Maximum number of pre-presentation guest swap sources to capture")
    .range(1, 1000);
...
bool WriteDebugSwapSourceBmp(const DebugSwapSourceCapture& capture) {
  constexpr uint32_t kBitmapHeaderSize = 14;
  constexpr uint32_t kDibHeaderSize = 40;
  constexpr uint32_t kHeaderSize = kBitmapHeaderSize + kDibHeaderSize;
  constexpr uint32_t kBytesPerPixel = 4;
  const uint64_t pixel_data_size = uint64_t(capture.width) * capture.height * kBytesPerPixel;
  if (!capture.width || !capture.height ||
      pixel_data_size > std::numeric_limits<uint32_t>::max() - kHeaderSize) {
    return false;
  }

  std::error_code error;
  if (!capture.output_path.parent_path().empty()) {
    std::filesystem::create_directories(capture.output_path.parent_path(), error);
    if (error) {
      return false;
    }
  }
  ...
  read_range.Begin = capture.footprint.Offset;
  read_range.End = capture.footprint.Offset +
                   size_t(capture.footprint.Footprint.RowPitch) * capture.height;
  void* mapping = nullptr;
  if (FAILED(capture.readback_resource->Map(0, &read_range, &mapping))) {
    return false;
  }

  std::ofstream stream(capture.output_path, std::ios::binary | std::ios::trunc);
  if (!stream) {
    capture.readback_resource->Unmap(0, nullptr);
    return false;
  }

  WriteLittleEndian16(stream, 0x4D42);
  WriteLittleEndian32(stream, kHeaderSize + uint32_t(pixel_data_size));
  WriteLittleEndian16(stream, 0);
  WriteLittleEndian16(stream, 0);
  WriteLittleEndian32(stream, kHeaderSize);
  WriteLittleEndian32(stream, kDibHeaderSize);
  WriteLittleEndian32(stream, capture.width);
  WriteLittleEndian32(stream, capture.height);
  WriteLittleEndian16(stream, 1);
  WriteLittleEndian16(stream, 32);
  WriteLittleEndian32(stream, 0);
  WriteLittleEndian32(stream, uint32_t(pixel_data_size));
  WriteLittleEndian32(stream, 0);
  WriteLittleEndian32(stream, 0);
  WriteLittleEndian32(stream, 0);
  WriteLittleEndian32(stream, 0);

  std::vector<uint8_t> converted_row(size_t(capture.width) * kBytesPerPixel);
  const uint8_t* source_base =
      reinterpret_cast<const uint8_t*>(mapping) + capture.footprint.Offset;
  for (uint32_t y = capture.height; y--;) {
    const uint8_t* source_row =
        source_base + size_t(y) * capture.footprint.Footprint.RowPitch;
    for (uint32_t x = 0; x < capture.width; ++x) {
      const uint8_t* source_pixel = source_row + size_t(x) * kBytesPerPixel;
      uint8_t* destination_pixel = converted_row.data() + size_t(x) * kBytesPerPixel;
      destination_pixel[0] = source_pixel[2];
      destination_pixel[1] = source_pixel[1];
      destination_pixel[2] = source_pixel[0];
      destination_pixel[3] = 0xFF;
    }
    stream.write(reinterpret_cast<const char*>(converted_row.data()),
                 std::streamsize(converted_row.size()));
  }

  D3D12_RANGE write_range = {};
  capture.readback_resource->Unmap(0, &write_range);
  return bool(stream);
}

std::filesystem::path GetDebugSwapCapturePath(const std::filesystem::path& base_path,
                                               uint32_t capture_index, bool sequence) {
  if (!sequence) {
    return base_path;
  }
  return base_path.parent_path() /
         (base_path.stem().string() + "_" + std::to_string(capture_index) +
...
void D3D12CommandProcessor::IssueSwap(uint32_t frontbuffer_ptr, uint32_t frontbuffer_width,
  ...
  DebugSwapSourceCapture debug_swap_capture;
  const std::string& debug_swap_capture_path = REXCVAR_GET(d3d12_debug_swap_capture_path);
  if (!debug_swap_capture_path.empty() && frontbuffer_format == xenos::TextureFormat::k_8_8_8_8 &&
      (swap_texture_desc.Format == DXGI_FORMAT_R8G8B8A8_TYPELESS ||
       swap_texture_desc.Format == DXGI_FORMAT_R8G8B8A8_UNORM ||
       swap_texture_desc.Format == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB)) {
    static uint64_t debug_swap_capture_count = 0;
    static uint64_t debug_swap_capture_next = 0;
    static uint32_t debug_swap_capture_emitted = 0;
    ++debug_swap_capture_count;
    const uint64_t debug_swap_capture_after = std::max<int32_t>(
        1, REXCVAR_GET(d3d12_debug_swap_capture_after));
    const int32_t debug_swap_capture_interval =
        REXCVAR_GET(d3d12_debug_swap_capture_interval);
    const uint32_t debug_swap_capture_limit = uint32_t(REXCVAR_GET(d3d12_debug_swap_capture_limit));
    if (!debug_swap_capture_next) {
      debug_swap_capture_next = debug_swap_capture_after;
    }
    if (debug_swap_capture_emitted < debug_swap_capture_limit &&
        debug_swap_capture_count >= debug_swap_capture_next) {
      const ui::d3d12::D3D12Provider& provider = GetD3D12Provider();
      uint64_t readback_size = 0;
      provider.GetDevice()->GetCopyableFootprints(&swap_texture_desc, 0, 1, 0,
                                                   &debug_swap_capture.footprint, nullptr, nullptr,
                                                   &readback_size);
      ...
      if (SUCCEEDED(provider.GetDevice()->CreateCommittedResource(
              ...
              IID_PPV_ARGS(&debug_swap_capture.readback_resource)))) {
        debug_swap_capture.width = source_width_scaled;
        debug_swap_capture.height = source_height_scaled;
        const bool debug_swap_capture_sequence =
            debug_swap_capture_interval > 0 && debug_swap_capture_limit > 1;
        debug_swap_capture.output_path = GetDebugSwapCapturePath(
            std::filesystem::path(debug_swap_capture_path), debug_swap_capture_emitted + 1,
            debug_swap_capture_sequence);
        ++debug_swap_capture_emitted;
        debug_swap_capture_next = debug_swap_capture_interval > 0
                                      ? debug_swap_capture_count + uint64_t(debug_swap_capture_interval)
                                      : std::numeric_limits<uint64_t>::max();
      } else {
        REXGPU_ERROR("IssueSwap: Failed to allocate the pre-presentation diagnostic readback");
      }
    }
  }

  const int32_t debug_swap_trace_interval = REXCVAR_GET(d3d12_debug_swap_trace_interval);
  if (debug_swap_trace_interval > 0) {
    static uint64_t debug_swap_count = 0;
    ++debug_swap_count;
    if (debug_swap_count <= 3 ||
        !(debug_swap_count % uint64_t(debug_swap_trace_interval))) {
      REXGPU_INFO(
          "IssueSwap #{}: packet={}x{} ptr={:08X}, source={}x{} native={}x{}, output={}x{}, "
          "xenos_format={}, dxgi_format={}",
          debug_swap_count, frontbuffer_width, frontbuffer_height, frontbuffer_ptr,
          source_width_scaled, source_height_scaled, frontbuffer_width_unscaled,
  ...
  presenter->RefreshGuestOutput(
      ...
      [this, &swap_texture_srv_desc, frontbuffer_format, swap_texture_resource, guest_output_width,
       guest_output_height, &debug_swap_capture](ui::Presenter::GuestOutputRefreshContext& context) -> bool {
        const ui::d3d12::D3D12Provider& provider = GetD3D12Provider();
        ID3D12Device* device = provider.GetDevice();
        ...
        // queue.
        SubmitBarriers();
        if (debug_swap_capture.readback_resource) {
          PushTransitionBarrier(swap_texture_resource, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                                D3D12_RESOURCE_STATE_COPY_SOURCE);
          SubmitBarriers();
          D3D12_TEXTURE_COPY_LOCATION destination = {};
          destination.pResource = debug_swap_capture.readback_resource.Get();
          destination.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
          destination.PlacedFootprint = debug_swap_capture.footprint;
          D3D12_TEXTURE_COPY_LOCATION source = {};
          source.pResource = swap_texture_resource;
          source.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
          source.SubresourceIndex = 0;
          deferred_command_list_.D3DCopyTextureRegion(&destination, 0, 0, 0, &source, nullptr);
          PushTransitionBarrier(swap_texture_resource, D3D12_RESOURCE_STATE_COPY_SOURCE,
                                D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
          SubmitBarriers();
          debug_swap_capture.copied = true;
        }
        EndSubmission(true);
        return true;
      });
  ...
  if (debug_swap_capture.copied) {
    if (!AwaitAllQueueOperationsCompletion() || !WriteDebugSwapSourceBmp(debug_swap_capture)) {
      REXGPU_ERROR("IssueSwap: Failed to write the pre-presentation diagnostic capture {}",
                   debug_swap_capture.output_path.string());
    } else {
      REXGPU_INFO("IssueSwap: Wrote {}x{} pre-presentation diagnostic capture to {}",
                  debug_swap_capture.width, debug_swap_capture.height,
                  debug_swap_capture.output_path.string());
    }
  }
}

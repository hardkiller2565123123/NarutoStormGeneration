...
REXCVAR_DEFINE_BOOL(d3d12_async_pipeline_wait_at_submission, false, "GPU/D3D12",
                    "Wait for background pipeline creation at each submission")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);
...
void PipelineCache::EndSubmission() {
  ...
  // The draw path deliberately skips unfinished async pipelines and retries on
  // a later frame. Waiting here defeats that behavior and turns first-time
  // effects into long frame stalls. Keep the old blocking behavior available
  // only as an opt-in compatibility fallback.
  if (!creation_threads_.empty() && REXCVAR_GET(d3d12_async_pipeline_wait_at_submission)) {
    CreateQueuedPipelinesOnProcessorThread();

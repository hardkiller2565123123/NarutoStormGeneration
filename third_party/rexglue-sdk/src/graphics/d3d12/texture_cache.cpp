...
std::unique_ptr<TextureCache::Texture> D3D12TextureCache::CreateTexture(TextureKey key) {
  ...
  HRESULT create_result =
      device->CreateCommittedResource(&ui::d3d12::util::kHeapPropertiesDefault,
                                      provider.GetHeapFlagCreateNotZeroed(), &desc,
                                      resource_state, nullptr,
                                      IID_PPV_ARGS(&resource));
  if (FAILED(create_result)) {
    // Character portraits disappearing before the next-match black screen is
    // consistent with a host texture allocation failing under accumulated
    // cache pressure. Ask the command processor to perform its synchronized
    // end-of-frame cache reset so the next fetch can recreate the texture
    // safely after GPU work has completed.
    REXGPU_ERROR(
        "D3D12TextureCache: texture allocation failed (HRESULT {:08X}, "
        "{}x{} mips={} array={}); requesting synchronized cache recovery",
        uint32_t(create_result), uint32_t(desc.Width), desc.Height,

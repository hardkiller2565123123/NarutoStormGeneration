...
void GraphicsSystem::NotifyGuestSwap() {
  const uint64_t frequency = chrono::Clock::QueryHostTickFrequency();
  if (!frequency) {
    return;
  }

  const uint64_t now = chrono::Clock::QueryHostTickCount();
  guest_swap_count_.fetch_add(1, std::memory_order_relaxed);

  if (guest_swap_last_tick_) {
    const uint64_t elapsed_ticks = now - guest_swap_last_tick_;
    const double elapsed_us =
        static_cast<double>(elapsed_ticks) * 1000000.0 / static_cast<double>(frequency);
    guest_swap_frame_time_us_.store(static_cast<uint64_t>(elapsed_us),
                                    std::memory_order_relaxed);
  }
  guest_swap_last_tick_ = now;

  if (!guest_swap_window_start_tick_) {
    guest_swap_window_start_tick_ = now;
  }
  ++guest_swap_window_count_;

  const uint64_t window_ticks = now - guest_swap_window_start_tick_;
  if (window_ticks >= frequency) {
    const double fps = static_cast<double>(guest_swap_window_count_) *
                       static_cast<double>(frequency) / static_cast<double>(window_ticks);
    guest_swap_fps_milli_.store(static_cast<uint64_t>(fps * 1000.0),
                                std::memory_order_relaxed);
    guest_swap_window_start_tick_ = now;
    guest_swap_window_count_ = 0;
  }
}

GuestSwapStats GraphicsSystem::guest_swap_stats() const {
  GuestSwapStats stats;
  stats.frame_count = guest_swap_count_.load(std::memory_order_relaxed);
  stats.fps = static_cast<double>(guest_swap_fps_milli_.load(std::memory_order_relaxed)) /
              1000.0;
  stats.frame_time_ms =
      static_cast<double>(guest_swap_frame_time_us_.load(std::memory_order_relaxed)) / 1000.0;
  return stats;
}
  ...
  vsync_worker_thread_ = system::object_ref<system::XHostThread>(
      new system::XHostThread(kernel_state_, 128 * 1024, 0, [this]() {
        ...
        while (vsync_worker_running_) {
          ...
          // The host presenter may run without VSync, but the Xbox title must
          // always receive vblanks at the configured guest refresh rate.
          // Tying this to host VSync made fixed-timestep games advance at
          // roughly 1000 Hz whenever VSync was disabled.

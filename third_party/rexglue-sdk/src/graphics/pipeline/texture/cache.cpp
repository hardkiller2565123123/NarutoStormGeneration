...
void TextureCache::CompletedSubmissionUpdated(uint64_t completed_submission_index) {
  ...
  bool recovering_from_hard_pressure = false;
  while (texture_used_first_ != nullptr) {
    uint64_t total_host_memory_usage_mb =
        (textures_total_host_memory_usage_ + ((UINT32_C(1) << 20) - 1)) >> 20;
    const bool limit_hard_exceeded = total_host_memory_usage_mb > limit_hard_mb;
    recovering_from_hard_pressure =
        recovering_from_hard_pressure || limit_hard_exceeded;
    if (total_host_memory_usage_mb <= limit_soft_mb) {
      break;
    }
    ...
    // Normal soft-limit cleanup respects texture age. After the hard limit has
    // been crossed, however, keep reclaiming completed resources until the
    // cache reaches the soft target. Stopping immediately below the hard limit
    // causes repeated allocation failures while character and stage packages
    // are streamed between matches.
    if (!recovering_from_hard_pressure &&

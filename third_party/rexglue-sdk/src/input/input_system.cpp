...
void InputSystem::Shutdown() {
  SetStateTransformCallback({});
  SetKeystrokeTransformCallback({});
  drivers_.clear();
}
...
void InputSystem::SetStateTransformCallback(StateTransformCallback callback) {
  std::scoped_lock lock(state_transform_callback_mutex_);
  state_transform_callback_ = std::move(callback);
}

void InputSystem::SetKeystrokeTransformCallback(KeystrokeTransformCallback callback) {
  std::scoped_lock lock(keystroke_transform_callback_mutex_);
  keystroke_transform_callback_ = std::move(callback);
}
...
X_RESULT InputSystem::GetState(uint32_t user_index, X_INPUT_STATE* out_state) {
  ...
  StateTransformCallback state_transform_callback;
  {
    std::scoped_lock lock(state_transform_callback_mutex_);
    state_transform_callback = state_transform_callback_;
  }
  if (state_transform_callback) {
    state_transform_callback(user_index, merged);
  }
...
X_RESULT InputSystem::GetKeystroke(uint32_t user_index, uint32_t flags,
  ...
  for (auto& driver : drivers_) {
    ...
    if (result == X_ERROR_SUCCESS) {
      KeystrokeTransformCallback keystroke_transform_callback;
      {
        std::scoped_lock lock(keystroke_transform_callback_mutex_);
        keystroke_transform_callback = keystroke_transform_callback_;
      }
      if (out_keystroke && keystroke_transform_callback &&
          keystroke_transform_callback(user_index, *out_keystroke)) {
        // The native overlay handled this event, so don't leak its shortcut
        // into the title's menu code.

...
REXCVAR_DEFINE_BOOL(mnk_headless_auto_confirm, false, "Input/Debug",
                    "For hidden diagnostics only, send short A/Start pulses every few seconds")
...
bool IsHeadlessAutoConfirmPulse(std::chrono::steady_clock::time_point started) {
  constexpr auto kFirstPressDelay = std::chrono::seconds(2);
  constexpr auto kPressPeriod = std::chrono::seconds(2);
  constexpr auto kPressDuration = std::chrono::milliseconds(250);
  const auto elapsed = std::chrono::steady_clock::now() - started;
  return elapsed >= kFirstPressDelay &&
         (elapsed - kFirstPressDelay) % kPressPeriod < kPressDuration;
}
...
X_STATUS MnkInputDriver::Setup() {
  auto_confirm_started_ = std::chrono::steady_clock::now();
...
X_RESULT MnkInputDriver::GetState(uint32_t user_index, X_INPUT_STATE* out_state) {
  ...
  const bool headless_auto_confirm = REXCVAR_GET(mnk_headless_auto_confirm);
  if (!is_active() || (!has_focus_ && !headless_auto_confirm)) {
    if (out_state) {
  ...
  if (headless_auto_confirm) {
    const bool pulse_active = IsHeadlessAutoConfirmPulse(auto_confirm_started_);
    if (pulse_active) {
      buttons |= X_INPUT_GAMEPAD_A | X_INPUT_GAMEPAD_START;
      if (!auto_confirm_pulse_active_) {
        REXLOG_INFO("MnK headless auto-confirm state pulse: buttons={:04X}", buttons);
      }
    }
    auto_confirm_pulse_active_ = pulse_active;
  }
...
X_RESULT MnkInputDriver::GetKeystroke(uint32_t user_index, uint32_t flags,
  ...
  std::lock_guard lock(state_mutex_);
  if (REXCVAR_GET(mnk_headless_auto_confirm)) {
    const bool pulse_active = IsHeadlessAutoConfirmPulse(auto_confirm_started_);
    if (pulse_active && !auto_confirm_keystroke_queued_) {
      REXLOG_INFO("MnK headless auto-confirm keystroke pulse");
      EnqueueKeystroke(uint16_t(VirtualKey::kXInputPadA), true);
      EnqueueKeystroke(uint16_t(VirtualKey::kXInputPadA), false);
      EnqueueKeystroke(uint16_t(VirtualKey::kXInputPadStart), true);
      EnqueueKeystroke(uint16_t(VirtualKey::kXInputPadStart), false);
    }
    auto_confirm_keystroke_queued_ = pulse_active;
  }

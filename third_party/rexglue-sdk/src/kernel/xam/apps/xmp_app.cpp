...
REXCVAR_DEFINE_BOOL(xmp_broadcast_controller_changed, true, "Kernel",
                    "Broadcast XMP playback-controller notifications")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);
REXCVAR_DEFINE_BOOL(xmp_set_playback_controller_fail, false, "Kernel/Debug",
                    "Return failure from XMPSetPlaybackController for title-flow diagnostics")
    .debug_only();
...
X_HRESULT XmpApp::DispatchMessageSync(uint32_t message, uint32_t buffer_ptr,
  ...
  switch (message) {
    ...
    case 0x0007001A: {
      ...
      if (REXCVAR_GET(xmp_set_playback_controller_fail)) {
        REXKRNL_INFO("XMPSetPlaybackController: diagnostic failure override active");
        return X_E_FAIL;
      }

      playback_client_ = PlaybackClient(uint32_t(args->playback_client));
      if (REXCVAR_GET(xmp_broadcast_controller_changed)) {
        kernel_state_->BroadcastNotification(kMsgPlaybackControllerChanged,
                                             !args->playback_client);
      } else {
        REXKRNL_INFO("XMPSetPlaybackController: suppressed controller-changed notification");
      }
      return X_E_SUCCESS;
    }
    case 0x0007001B: {

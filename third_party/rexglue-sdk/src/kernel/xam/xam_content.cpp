...
u32 XamContentCreateEnumerator_entry(u32 user_index, u32 device_id, u32 content_type,
  ...
  REXKRNL_INFO(
      "SaveEnum: XamContentCreateEnumerator user={} device=0x{:08X} "
      "type=0x{:08X} flags=0x{:08X} requested={} active_xuid=0x{:016X} "
      "title=0x{:08X}",
      user_index, device_id, content_type, content_flags, items_per_enumerate,
      xuid, REX_KERNEL_STATE()->title_id());
  ...
  if (!device_info || device_info->device_id == DummyDeviceId::HDD) {
    ...
    REXKRNL_INFO("SaveEnum: user-specific query returned {} item(s)",
                 content_datas.size());
    for (const auto& content_data : content_datas) {
      REXKRNL_INFO(
          "SaveEnum: user item type=0x{:08X} title=0x{:08X} "
          "xuid=0x{:016X} file='{}' display='{}'",
          static_cast<uint32_t>(content_data.content_type.get()),
          uint32_t(content_data.title_id),
          uint64_t(content_data.xuid), content_data.file_name(),
    ...
    if (xuid != 0) {
      ...
      REXKRNL_INFO("SaveEnum: common query returned {} item(s)",
                   common_datas.size());
      for (const auto& content_data : common_datas) {
        REXKRNL_INFO(
            "SaveEnum: common item type=0x{:08X} title=0x{:08X} "
            "xuid=0x{:016X} file='{}' display='{}'",
            static_cast<uint32_t>(content_data.content_type.get()),
          uint32_t(content_data.title_id),
            uint64_t(content_data.xuid), content_data.file_name(),
  ...
  REXKRNL_INFO("SaveEnum: enumerator handle will contain {} item(s)",
               e->item_count());
...
u32 xeXamContentCreate(u32 user_index, mapped_string root_name, mapped_void content_data_ptr,
  ...
  REXKRNL_INFO(
      "SaveOpen: XamContentCreate user={} root='{}' flags=0x{:08X} "
      "type=0x{:08X} title=0x{:08X} active_xuid=0x{:016X} "
      "data_xuid=0x{:016X} file='{}' display='{}'",
      user_index, root_name.value(), flags,
      static_cast<uint32_t>(content_data.content_type.get()),
      uint32_t(content_data.title_id), xuid, uint64_t(content_data.xuid),
      content_data.file_name(), rex::string::to_utf8(content_data.display_name()));
  ...
  auto run = [content_manager, xuid, root_name = root_name.value(), flags, content_data,
    ...
    REXKRNL_INFO(
        "SaveOpen complete: root='{}' file='{}' result=0x{:08X} "
        "disposition={} extended=0x{:08X}",
        root_name, content_data.file_name(), uint32_t(result),
        static_cast<uint32_t>(disposition), extended_error);
    return result;
  };

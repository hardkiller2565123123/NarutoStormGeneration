...
bool ShouldLogInputCall(uint64_t call) {
  return call <= 16 || (call % 300) == 0;
}
...
u32 XamInputGetState_entry(u32 user_index, u32 flags, ppc_ptr_t<X_INPUT_STATE> input_state) {
  ...
  if (ShouldLogInputCall(call)) {
    const bool has_state = result == X_ERROR_SUCCESS && input_state;
    const uint16_t buttons =
        has_state ? static_cast<uint16_t>(input_state->gamepad.buttons) : 0;
    const uint32_t packet = has_state ? static_cast<uint32_t>(input_state->packet_number) : 0;
    REXKRNL_INFO("[XAM] XamInputGetState #{} user={} actual={} flags=0x{:X} state={:08X} result=0x{:08X} packet={} buttons=0x{:04X}",
                 call, (uint32_t)user_index, actual_user_index, (uint32_t)flags,
                 input_state.guest_address(), result, packet, buttons);
  }
  return result;
}
...
u32 XamInputGetKeystroke_entry(u32 user_index, u32 flags, ppc_ptr_t<X_INPUT_KEYSTROKE> keystroke) {
  ...
  auto* is = input_system();
  const u32 result = is->GetKeystroke(actual_user_index, flags, keystroke);
  if (ShouldLogInputCall(call) || result == X_ERROR_SUCCESS) {
    const bool has_key = result == X_ERROR_SUCCESS && keystroke;
    const uint16_t vk = has_key ? static_cast<uint16_t>(keystroke->virtual_key) : 0;
    const uint16_t ks_flags = has_key ? static_cast<uint16_t>(keystroke->flags) : 0;
    REXKRNL_INFO("[XAM] XamInputGetKeystroke #{} user={} actual={} flags=0x{:X} key={:08X} result=0x{:08X} vk=0x{:04X} ks_flags=0x{:04X}",
                 call, (uint32_t)user_index, actual_user_index, (uint32_t)flags,
                 keystroke.guest_address(), result, vk, ks_flags);
  }
  return result;
}
...
u32 XamInputGetKeystrokeEx_entry(mapped_u32 user_index_ptr, u32 flags,
  ...
  if (ShouldLogInputCall(call) || result == X_ERROR_SUCCESS) {
    ...
    const uint16_t vk = has_key ? static_cast<uint16_t>(keystroke->virtual_key) : 0;
    const uint16_t ks_flags = has_key ? static_cast<uint16_t>(keystroke->flags) : 0;
    REXKRNL_INFO("[XAM] XamInputGetKeystrokeEx #{} user={} flags=0x{:X} key={:08X} result=0x{:08X} vk=0x{:04X} ks_flags=0x{:04X}",
                 call, user_index, (uint32_t)flags, keystroke.guest_address(), result, vk,
                 ks_flags);
  }

...
uint32_t xeXamNotifyCreateListener(uint64_t mask, uint32_t is_system, uint32_t max_version) {
  ...
  REXKRNL_INFO(
      "[NOTIFYTRACE] XamNotifyCreateListener mask={:016X} system={} max_version={} handle={:08X}",
...
u32 XNotifyGetNext_entry(u32 handle, u32 match_id, mapped_u32 id_ptr, mapped_u32 param_ptr) {
  ...
  const uint64_t poll = ++poll_count;
  if (param_ptr) {
    *param_ptr = 0;
  }

  if (!id_ptr) {
    REXKRNL_INFO("[NOTIFYTRACE] XNotifyGetNext poll={} handle={:08X} match={:08X} result=0 reason=null-id",
                 poll, static_cast<uint32_t>(handle), static_cast<uint32_t>(match_id));
    return 0;
  }
  *id_ptr = 0;
  ...
  if (!listener) {
    REXKRNL_INFO("[NOTIFYTRACE] XNotifyGetNext poll={} handle={:08X} match={:08X} result=0 reason=invalid-listener",
                 poll, static_cast<uint32_t>(handle), static_cast<uint32_t>(match_id));
    return 0;
  }
  ...
  *id_ptr = dequeued ? id : 0;
  ...
  if (dequeued) {
    REXKRNL_INFO("[NOTIFYTRACE] XNotifyGetNext poll={} handle={:08X} match={:08X} result=1 id={:08X} param={:08X}",
                 poll, static_cast<uint32_t>(handle), static_cast<uint32_t>(match_id), id, param);
  } else {
    const uint64_t empty = ++empty_count;
    // Empty polling can occur every frame. Log the first 32 misses, then only
    // periodic samples so diagnostics remain useful without stalling the game.
    if (empty <= 32 || (empty % 300) == 0) {
      REXKRNL_INFO("[NOTIFYTRACE] XNotifyGetNext poll={} empty={} handle={:08X} match={:08X} result=0",
                   poll, empty, static_cast<uint32_t>(handle),
                   static_cast<uint32_t>(match_id));
    }
  }
  return dequeued ? 1 : 0;
}

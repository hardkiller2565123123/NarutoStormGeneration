...
u32 XamShowMessageBoxUI_entry(u32 user_index, mapped_wstring title_ptr, mapped_wstring text_ptr,
  ...
  const auto load_guest_wide = [](mapped_wstring value) -> std::string {
    if (!value) {
      return {};
    }
    const auto swapped = rex::memory::load_and_swap<std::u16string>(
        REX_KERNEL_MEMORY()->TranslateVirtual(value.guest_address()));
    return rex::string::to_utf8(swapped);
  };

  std::string title = load_guest_wide(title_ptr);
  const std::string text = load_guest_wide(text_ptr);
  ...
  REXKRNL_INFO(
      "MessageBoxUI show: title='{}' text='{}' buttons={} active={} "
      "overlapped={:08X}",
      title, text, buttons.size(), active_button, overlapped.guest_address());
  ...
  if (REXCVAR_GET(headless)) {
    ...
    auto close = [result_ptr, title](MessageBoxDialog* dialog) -> X_RESULT {
      const uint32_t chosen_button = dialog->chosen_button();
      *result_ptr = chosen_button;
      REXKRNL_INFO("MessageBoxUI close: title='{}' selected={}", title,
                   chosen_button);
      return X_ERROR_SUCCESS;
    };
...
u32 XamShowPartyUI_entry(u32 r3, u32 r4) {
  REXKRNL_WARN("XamShowPartyUI requested while Xbox Live UI is unavailable; cancelling safely");
  return X_ERROR_CANCELLED;
}

u32 XamShowCommunitySessionsUI_entry(u32 r3, u32 r4) {
  REXKRNL_WARN(
      "XamShowCommunitySessionsUI requested while Xbox Live UI is unavailable; cancelling safely");
  return X_ERROR_CANCELLED;
}


u32 XamShowFriendsUI_entry(u32 user_index) {
  // There is no platform guide in the recomp runtime, but Generations calls
  // this from its online lobby. A deterministic successful no-op is safer than
  // leaving r3 untouched through REX_EXPORT_STUB, which can look like a random
  // error code to the PPC caller.
  REXKRNL_INFO(
      "[XEXONLINE][UI] XamShowFriendsUI user={} -> local System Link no-op",
      user_index);
  REX_KERNEL_STATE()->BroadcastNotification(0x9, true);
  REX_KERNEL_STATE()->BroadcastNotification(0x9, false);
  return X_ERROR_SUCCESS;
...
u32 XamShowFriendsUIp_entry(u32 user_index, u32 unknown) {
  REXKRNL_INFO(
      "[XEXONLINE][UI] XamShowFriendsUIp user={} unknown={:08X} -> local System Link no-op",
      user_index, unknown);
  REX_KERNEL_STATE()->BroadcastNotification(0x9, true);
  REX_KERNEL_STATE()->BroadcastNotification(0x9, false);
  return X_ERROR_SUCCESS;
...
uint32_t XamShowMessageBoxUIEx_entry() {
  // Extended message-box layouts are not needed by the Generations front end,
  // but the call must still have deterministic UI lifecycle behavior.
  REXKRNL_INFO("[XEXONLINE][UI] XamShowMessageBoxUIEx -> headless acknowledgement");
  REX_KERNEL_STATE()->BroadcastNotification(0x9, true);
  REX_KERNEL_STATE()->BroadcastNotification(0x9, false);
  return X_ERROR_SUCCESS;
}
...
REX_EXPORT(__imp__XamShowFriendsUIp, rex::kernel::xam::XamShowFriendsUIp_entry)

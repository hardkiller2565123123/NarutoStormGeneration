...
u32 NtQueryInformationFile_entry(u32 file_handle, ppc_ptr_t<X_IO_STATUS_BLOCK> io_status_block_ptr,
  ...
  switch (info_class) {
    ...
    case XFileXctdCompressionInformation: {
      ...
      size_t bytes_read = 0;
      status = file->file()->ReadSync(magic, 0, &bytes_read);
      if (XSUCCEEDED(status) && bytes_read == magic.size()) {
        const bool is_compressed =
            magic == std::array<uint8_t, 4>{0x0F, 0xF5, 0x12, 0xED};
        auto info = info_ptr.as<X_FILE_XCTD_COMPRESSION_INFORMATION*>();
        info->unknown = is_compressed ? 1u : 0u;
        out_length = sizeof(*info);
        REXKRNL_DEBUG("XCTD compression query: '{}' -> {}", file->path(),
                      is_compressed ? "compressed" : "plain");
      } else {

...
u32 ObDereferenceObject_entry(u32 native_ptr) {
  ...
  auto* native_data = REX_KERNEL_MEMORY()->TranslateVirtual(native_ptr);
  // XamSessionRefObjByHandle returns a small XAM-native session object whose
  // first word is the session handle, not an ordinary dispatcher header. If it
  // is decoded by GetNativeObject, the handle bytes are mistaken for header
  // fields and the lobby path can lose the real session object right after
  // XSessionJoinLocal.
  const uint32_t possible_session_handle = *reinterpret_cast<rex::be<uint32_t>*>(native_data);
  auto possible_session = REX_KERNEL_OBJECTS()->LookupObject<XObject>(possible_session_handle);
  if (possible_session && possible_session->type() == XObject::Type::Session) {
    REXKRNL_INFO("ObDereferenceObject(session token {:08X}, handle={:08X})", native_ptr,
                 possible_session_handle);
    // XamSessionRefObjByHandle returns a title-facing token backed by the
    // existing session handle. Do not release the actual handle here; the title
    // still owns it and will close it explicitly when leaving the lobby.
    return 0;
  }

  auto object = XObject::GetNativeObject<XObject>(REX_KERNEL_STATE(), native_data);
  if (object) {
    if (object->type() == XObject::Type::Session) {
      REXKRNL_INFO("ObDereferenceObject(session {:08X}, handle={:08X})", native_ptr,
                   object->handle());
    }
...
u32 NtClose_entry(u32 handle) {
  ...
  if (object && object->type() == XObject::Type::Session) {
    REXKRNL_INFO("NtClose(session handle={:08X})", handle);
  }

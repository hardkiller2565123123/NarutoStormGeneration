...
REXCVAR_DEFINE_BOOL(kernel_semaphore_trace, false, "Kernel",
                    "Trace guest semaphore initialize and release operations")
    .debug_only();
...
void KeInitializeSemaphore_entry(ppc_ptr_t<X_KSEMAPHORE> semaphore_ptr, u32 count, u32 limit) {
  ...
  if (REXCVAR_GET(kernel_semaphore_trace)) {
    REXKRNL_INFO("[SEMTRACE] KeInitializeSemaphore ptr={:08X} handle={:08X} count={} limit={}",
                 semaphore_ptr.guest_address(), sem->handle(), count, limit);
  }
}
...
uint32_t xeKeReleaseSemaphore(X_KSEMAPHORE* semaphore_ptr, uint32_t increment, uint32_t adjustment,
  ...
  if (REXCVAR_GET(kernel_semaphore_trace)) {
    REXKRNL_INFO(
        "[SEMTRACE] KeReleaseSemaphore ptr={:08X} handle={:08X} inc={} adj={} wait={} prev={} success={}",
        REX_KERNEL_MEMORY()->HostToGuestVirtual(semaphore_ptr), sem->handle(), increment,
        adjustment, wait, previous_count, success);
  }
...
u32 NtCreateSemaphore_entry(mapped_u32 handle_ptr, mapped_void obj_attributes_ptr, u32 count,
  ...
  if (REXCVAR_GET(kernel_semaphore_trace)) {
    REXKRNL_INFO("[SEMTRACE] NtCreateSemaphore handle={:08X} count={} limit={} attrs={:08X}",
                 sem->handle(), count, limit, obj_attributes_ptr.guest_address());
  }
...
u32 NtReleaseSemaphore_entry(u32 sem_handle, u32 release_count, mapped_u32 previous_count_ptr) {
  ...
  if (sem) {
    ...
    if (REXCVAR_GET(kernel_semaphore_trace)) {
      REXKRNL_INFO(
          "[SEMTRACE] NtReleaseSemaphore handle={:08X} guest={:08X} release={} prev={} success={}",
          sem_handle, sem->guest_object(), release_count, previous_count, success);
    }
    if (!success) {

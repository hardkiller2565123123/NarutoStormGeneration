...
REXCVAR_DEFINE_STRING(game_mods_root, "", "Runtime",
                      "Read-only loose file overlay for the game data root");
...
bool Runtime::SetupVfs() {
  ...
  std::filesystem::path mods_root = REXCVAR_GET(game_mods_root);
  if (!mods_root.empty()) {
    mods_root = std::filesystem::absolute(mods_root);
  }

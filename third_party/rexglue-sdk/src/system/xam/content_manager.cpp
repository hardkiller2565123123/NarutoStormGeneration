...
X_RESULT ContentManager::OpenContent(const std::string_view root_name, uint64_t xuid,
  ...
  {
    auto global_lock = global_critical_region_.Acquire();
    auto existing = open_packages_.find(string::string_key_case(root_name));
    if (existing != open_packages_.end()) {
      const auto& mounted = existing->second->GetPackageContentData();
      const uint32_t requested_title =
          data.title_id == kCurrentlyRunningTitleId ? kernel_state_->title_id()
                                                   : uint32_t(data.title_id);
      const uint32_t mounted_title =
          mounted.title_id == kCurrentlyRunningTitleId
              ? kernel_state_->title_id()
              : uint32_t(mounted.title_id);
      if (requested_title == mounted_title &&
          data.content_type == mounted.content_type &&
          data.file_name() == mounted.file_name()) {
        // Titles may issue a second OPEN_EXISTING for the same logical save
        // before closing the first mount. The Xbox content service treats the
        // already-mounted root as usable; returning ALREADY_EXISTS here sends
        // Generations back into its first-save confirmation loop.
        content_license = existing->second->GetPackageLicense();
        REXLOG_INFO("Content root '{}' already mounts '{}'; reusing it",
...
X_RESULT ContentManager::InstallContent(const std::filesystem::path& package_path) {
  ...
  // Preserve the package's real metadata. Saved-game packages must be
  // installed under the active ReXGlue profile XUID and kSavedGame content
  // type or XamContentCreateEnumerator will never return them to the title.
  const auto& metadata = device->header().metadata;
  auto file_name = rex::path_to_utf8(package_path.filename());

  XCONTENT_AGGREGATE_DATA content_data;
  content_data.device_id = static_cast<uint32_t>(DummyDeviceId::HDD);
  content_data.content_type = metadata.content_type.get();
  content_data.title_id = uint32_t(metadata.execution_info.title_id);
  if (!content_data.title_id) {
    content_data.title_id = kernel_state_->title_id();
  }

  const uint64_t active_xuid = kernel_state_->user_profile()->xuid();
  content_data.xuid =
      content_data.content_type == XContentType::kMarketplaceContent ? 0 : active_xuid;
  ...
  const uint64_t install_xuid = uint64_t(content_data.xuid);
  auto install_path = ResolvePackagePath(install_xuid, content_data);

  REXLOG_INFO(
      "InstallContent: source='{}' type=0x{:08X} title=0x{:08X} "
      "package_profile=0x{:016X} active_xuid=0x{:016X} install_xuid=0x{:016X} "
      "destination='{}'",
      rex::path_to_utf8(package_path),
      static_cast<uint32_t>(content_data.content_type.get()),
      uint32_t(content_data.title_id), uint64_t(metadata.profile_id), active_xuid,
      install_xuid, rex::path_to_utf8(install_path));
  ...
  // Write .header file beside the same profile/title/content-type tree used
  // by XamContentCreateEnumerator.
  const auto header_result =
      WriteContentHeaderFile(install_xuid, content_data, license_mask);
  REXLOG_INFO("InstallContent: header result=0x{:08X} file='{}'",
              uint32_t(header_result), content_data.file_name());

...
REXCVAR_DEFINE_STRING(user_profile_name, "User", "Profile",
                      "Local profile name used by titles and system-link");
REXCVAR_DEFINE_STRING(user_profile_xuid, "B13EBABEBABEBABE", "Profile",
                      "Hexadecimal local profile XUID used by titles and system-link")
    .lifecycle(rex::cvar::Lifecycle::kInitOnly);
REXCVAR_DEFINE_STRING(user_profile_machine, "", "Profile",
                      "Owning machine key for the local player profile")
    .lifecycle(rex::cvar::Lifecycle::kInitOnly);
...
UserProfile::UserProfile() {
  ...
  const std::string configured_xuid = REXCVAR_GET(user_profile_xuid);
  std::string_view xuid_text = configured_xuid;
  if (xuid_text.starts_with("0x") || xuid_text.starts_with("0X")) {
    xuid_text.remove_prefix(2);
  }
  const auto conversion =
      std::from_chars(xuid_text.data(), xuid_text.data() + xuid_text.size(), xuid_, 16);
  if (conversion.ec != std::errc() || conversion.ptr != xuid_text.data() + xuid_text.size() ||
      !xuid_ || (xuid_ & kRejectedXuidMask) != 0) {
    REXSYS_WARN("Configured profile XUID '{}' is invalid; using the fallback identity",
                configured_xuid);
    xuid_ = kDefaultXuid;
  }
  ...
  REXSYS_INFO("User profile initialized: name='{}' xuid=0x{:016X}", name_, xuid_);

...
REXCVAR_DEFINE_BOOL(present_vsync, false, "UI/D3D12",
                    "Synchronize host presentation to the display refresh rate")
    .lifecycle(rex::cvar::Lifecycle::kHotReload);

REXCVAR_DEFINE_STRING(d3d12_debug_capture_guest_output_path, "", "UI/D3D12",
                      "Write one guest-output frame to this BMP path for a hidden diagnostic run");

REXCVAR_DEFINE_INT32(d3d12_debug_capture_guest_output_delay_seconds, 0, "UI/D3D12",
                     "Seconds to wait before a hidden guest-output diagnostic capture")
    .range(0, 600);
...
bool WriteGuestOutputBmp(const std::filesystem::path& output_path, const RawImage& image) {
  ...
  if (!image.width || !image.height ||
      image.stride < size_t(image.width) * kBytesPerPixel) {
    return false;
  }

  const uint64_t pixel_data_size = uint64_t(image.width) * image.height * kBytesPerPixel;
  if (pixel_data_size > std::numeric_limits<uint32_t>::max() - kHeaderSize) {
    return false;
  }
  ...
  std::vector<uint8_t> converted_row(size_t(image.width) * kBytesPerPixel);
  for (uint32_t y = image.height; y--;) {
    const uint8_t* source_row = image.data.data() + size_t(y) * image.stride;
...
void CaptureGuestOutputForDiagnostics(D3D12Presenter& presenter) {
  static std::mutex capture_mutex;
  static bool capture_scheduled = false;
  static bool capture_finished = false;
  static std::chrono::steady_clock::time_point capture_deadline;

  const std::string& capture_path = REXCVAR_GET(d3d12_debug_capture_guest_output_path);
  ...
  std::filesystem::path output_path;
  {
    ...
    if (!capture_scheduled) {
      capture_deadline =
          now + std::chrono::seconds(REXCVAR_GET(d3d12_debug_capture_guest_output_delay_seconds));
  ...
  RawImage image;
  if (!presenter.CaptureGuestOutput(image) || !WriteGuestOutputBmp(output_path, image)) {
    REXLOG_ERROR("D3D12Presenter: Failed to write guest-output diagnostic capture {}",
                 output_path.string());
    return;
  }
  REXLOG_INFO("D3D12Presenter: Wrote {}x{} guest-output diagnostic capture to {}", image.width,
              image.height, output_path.string());
}
...
Presenter::PaintResult D3D12Presenter::PaintAndPresentImpl(bool execute_ui_drawers) {
  ...
  // Host presentation can be synchronized independently of the guest timing.
  // This lets fixed-30-FPS titles remain animation-safe while users choose
  // either smooth, tear-free output or the original low-latency VRR path.
  const bool present_vsync = REXCVAR_GET(present_vsync);
  HRESULT present_result = paint_context_.swap_chain->Present(
      present_vsync ? 1 : 0,
      DXGI_PRESENT_RESTART |
          ((!present_vsync && paint_context_.swap_chain_allows_tearing)
  ...
  CaptureGuestOutputForDiagnostics(*this);
